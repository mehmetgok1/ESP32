...
void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_WRITE_DESCR_EVT:
        ...
        ESP_LOGI(GATTC_TAG, "write description success ");
    ...
    case ESP_GATTC_DISCONNECT_EVT:
        ...
        esp_err_t err = esp_ble_gattc_close(gl_profile_tab[PROFILE_A_APP_ID].gattc_if,gl_profile_tab[PROFILE_A_APP_ID].conn_id);
        
        // Show device list screen or handle post-disconnect logic
        lv_scr_load(device_list_screen);
        break;
    default:
        break;
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT:
            ...
            if(flag_connect==1){
                ...
                if(adv_name_len!=0){
                    ...
                    device_name[adv_name_len] = '\0';  // Null-terminate the string
                    if (device_count==0) {
                            added_devices[device_count] = strdup(device_name);
                            device_count++;
                    }else{
                        for (int i = 0; i < device_count; i++) {
                            if(strcmp(device_name, added_devices[i]) != 0 ){
                                // Allocate memory for the device name and copy it into the list
                                added_devices[device_count] = strdup(device_name);  // Use strdup to allocate memory
                                if (added_devices[device_count] == NULL) {
